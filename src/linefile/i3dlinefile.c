#include "../../inc/linefile/i3dlinefile.h"
...
static void fill_i3dLine(char *line, struct i3dLine *LI_origin,  long *lNum, int each, int *vtMaxId, int *vtMinId, int *_vtMaxId, int *_vtMinId, long filelineNum)
{
	...
	struct i3dLine *LI = LI_origin+linesNum;
...
{
	printf("read i3dLineFile %s: \n", filename);
	//open file
	FILE *fp=fopen(filename,"r");
	fileError(fp, filename);

	struct i3dLine *LinesInfo=NULL;
	LinesInfo=malloc(LINES_LENGTH_EACH*sizeof(struct i3dLine));
	...
	while(fgets(line, LINE_LENGTH_MAX, fp)) {
		...
		if (linesNum<LINES_LENGTH_EACH) {
			fill_i3dLine(line, LinesInfo, &linesNum, each, &maxId, &minId, &_maxId, &_minId, filelineNum);
		} else {
			++each;
			printf("\tread valid lines: %d\n", (each-1)*LINES_LENGTH_EACH); fflush(stdout);
			struct i3dLine *temp=realloc(LinesInfo, each*LINES_LENGTH_EACH*sizeof(struct i3dLine));
			assert(temp!=NULL);
			LinesInfo=temp;
			linesNum=0;
			fill_i3dLine(line, LinesInfo, &linesNum, each, &maxId, &minId, &_maxId, &_minId, filelineNum);
	...
	struct i3dLineFile *file=malloc(sizeof(struct i3dLineFile));
	assert(file!=NULL);
	file->i1Min=minId;
	file->i1Max=maxId;
	file->i2Min=_minId;
