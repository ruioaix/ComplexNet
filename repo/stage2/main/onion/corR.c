...
int main(int argc, char **argv)
{
	...
	for(i=0; i<101; ++i) {
		double corR = i*0.01;
		struct Metrics_Bipii *corR_result = mass_corR_Bipii(traini1, traini2, testi1, testi2, itemSim, userSim, corR);
		printf("corR\tR: %f, PL: %f, IL: %f, HL: %f, NL: %f, corR: %f\n", corR_result->R, corR_result->PL, corR_result->IL, corR_result->HL, corR_result->NL, corR);fflush(stdout);
		free_MetricsBipii(corR_result);
	}
