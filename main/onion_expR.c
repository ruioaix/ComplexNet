...
int main(int argc, char **argv)
{
	...
	for (j=0; j<50; ++j) {
		expR = j*0.1;
		struct Metrics_Bipii *expR_result = mass_expR_Bipii(traini1, traini2, testi1, testi2, itemSim, userSim, expR);
		printf("expR\tR: %f, PL: %f, IL: %f, HL: %f, NL: %f, expR: %f\n", expR_result->R, expR_result->PL, expR_result->IL, expR_result->HL, expR_result->NL, expR);fflush(stdout);
		free_MetricsBipii(expR_result);
	}
