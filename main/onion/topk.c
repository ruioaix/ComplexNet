...
int main(int argc, char **argv)
{
	...
	for(i=1; i<max_similaruer+1; ++i) {
		struct Metrics_iiBip *topk_result = mass_topk_iiBip(traini1, traini2, testi1, testi2, userSim, itemSim, i);
		printf("topk result =>> Rate: %f, topk: %d, R: %f, PL: %f, IL: %f, HL: %f, NL: %f\n", rate, i, topk_result->R, topk_result->PL, topk_result->IL, topk_result->HL, topk_result->NL);
		free_MetricsiiBip(topk_result);
	}
