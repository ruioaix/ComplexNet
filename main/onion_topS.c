...
int main(int argc, char **argv)
{
	...
	for(i=0; i<100; ++i) {
		double topS = i*0.01;
		struct Metrics_Bipii *topS_result = mass_topS_Bipii(traini1, traini2, testi1, testi2, itemSim, userSim, topS);
		printf("topS\tR: %f, PL: %f, IL: %f, HL: %f, NL: %f, topS: %f\n", topS_result->R, topS_result->PL, topS_result->IL, topS_result->HL, topS_result->NL, topS);fflush(stdout);
		free_MetricsBipii(topS_result);
	}
